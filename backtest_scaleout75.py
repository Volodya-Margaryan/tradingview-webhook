#!/usr/bin/env python3
"""Scale-out variant of the bracket-exit strategy, weighted 75/25 instead
of 50/50: three-quarters of each position is capped at a +2% partial
profit target, one-quarter is left uncapped to run with the trend.

  - Tranche A (75% of the position) takes a +2% partial profit if touched
    (checked intraday), locking in a realized win.
  - Tranche B (25% of the position) has no take-profit and exits only on
    the -3% stop or a trend reversal.

Both tranches share the same entry price/date and the same -3% stop. Same
mechanics as backtest_scaleout.py, with the split ratio and partial-profit
target adjusted to test how far win rate can be pushed before profit
factor breaks down. $2,000 total cap, 18-symbol universe, 14 periods,
10bps friction, intraday-aware High/Low checking.
"""
...
SMA_LONG = 50
STOP_LOSS_PCT = -0.03
PARTIAL_TARGET_PCT = 0.02  # tranche A only, tightened from 3% -> 2%
CAPPED_FRACTION = 0.75      # tranche A now gets 75% of the position, not 50%
...
def run_period(data: dict[str, pd.DataFrame], dates: list, db_path: Path, label: str) -> dict:
    ...
    def try_open(symbol: str, price: float, at_iso: str, today: pd.Timestamp) -> bool:
        ...
        if remaining < 1.0:
            return False
        position_dollars = min(SLICE_SIZE, remaining)
        if position_dollars <= 0:
            return False
        fill = buy_fill_price(price)
        tranches = []
        for tag, frac in (("A", CAPPED_FRACTION), ("B", 1 - CAPPED_FRACTION)):
            qty = round(position_dollars * frac / fill, 6)
            if qty <= 0:
                continue
            pid = storage.open_position(symbol, "long", qty, fill, f"scaleout75-{tag}-{label}", at=at_iso)
...
def main() -> None:
    ...
    print("\n" + "=" * 100)
    print("SCALE-OUT 75/25 (75% at +2% partial, 25% uncapped) -- same 14 periods, same $2,000 cap")
    print("=" * 100)
    ...
    print("(Compare to 50/50 scale-out @ +3%: 10/14 profitable, win rate 40.4%, +$475.87.)")
