...
def run_period(data: dict[str, pd.DataFrame], dates: list, db_path: Path, label: str) -> dict:
    ...
    for date in dates:
        ...
        for sym, h in data.items():
            ...
            pid = storage.open_position(sym, "long", qty, buy_fill, f"daytrade-{label}", at=date_iso)
...
def main() -> None:
    ...
    print("\n" + "=" * 100)
    print("DAY TRADING (gap-up continuation, same-day close only) -- 14 periods, $2,000 cap")
    print("=" * 100)
